...
public class VectorClock implements Serializable {
    ...
    public void updateClockRule2(VectorClock vci) {
		...
		for (int i = 0; i < vectorclock.size(); i++) {
            vectorclock.set(i, Math.max(vectorclock.get(i), vci.getVectorclock().get(i)));
        }
    }
    ...
    public boolean deliverMessage(VectorClock vi) {
        ...
        // implement
		if (clock_vi.get(pi) == clock_vj.get(pi) + 1) {
			expectedmsg = true;
		}
        ...
        // implement
		if (pi != index) {
			for (int i = 0; i < clock_vi.size(); i++) {
				if (i != pi && clock_vi.get(i) > clock_vj.get(i)) {
					prevmsgsdelivered = false;
					break;
				}
			}
		}
